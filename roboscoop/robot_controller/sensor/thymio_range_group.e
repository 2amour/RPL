...
feature {NONE} -- Initialization
	...
	initialize_sensors_transforms (range_sensors_parameters: separate RANGE_SENSORS_PARAMETERS)
		...
		do
			...
			from
				i := range_sensors_parameters.sensors_poses.lower
			until
				i > range_sensors_parameters.sensors_poses.upper
			loop
...
feature -- Access
	...
	transforms: ARRAY [TRANSFORM_2D]
			-- Transforms that map sensor relative coordinates to robot relative coordinates.
	...
	is_obstacle_in_front: BOOLEAN
		...
		local
			i: INTEGER
		do
				-- TODO - structure with front sensors?
			Result := sensors [1].is_valid_range or sensors [2].is_valid_range or sensors [3].is_valid_range or sensors [4].is_valid_range or sensors [5].is_valid_range
		end

	is_obstacle_at_back: BOOLEAN
			-- Whether an obstacle is observed at back.
		do
				-- TODO - structure with back sensors?
			Result := sensors [6].is_valid_range or sensors [7].is_valid_range
		end

	is_obstacle_mostly_at_left: BOOLEAN
			-- Whether an obstacle is observed mostly at left.
		local
			i: INTEGER
			left_sum, right_sum: REAL_32
		do
				-- TODO - structure with left and right sensors?
			from
				i := sensors.lower
			until
				i > sensors.upper
	...
	is_obstacle_mostly_at_right: BOOLEAN
			-- Whether an obstacle is observed mostly at right.
		local
			i: INTEGER
			left_sum, right_sum: REAL_32
		do
	...
	is_obstacle_huge: BOOLEAN
			-- Whether an obstacle is observed by majority of the sensors.
		do
				-- TODO - structure with front and back sensors?
			Result := (sensors [1].is_valid_range and sensors [2].is_valid_range and sensors [3].is_valid_range and sensors [4].is_valid_range and sensors [5].is_valid_range) or (sensors [6].is_valid_range and sensors [7].is_valid_range)
		end

	is_front_sensor (a_index: INTEGER): BOOLEAN
			-- Whether `a_index' represent a front sensor of the robot.
		do
			Result := a_index = 3
		end
	...
	has_obstacle (a_direction_with_respect_to_robot: REAL_64): BOOLEAN
		...
		do
			...
			across
				sensors as sensor
			loop
				Result := Result or (sensor.item.is_valid_range and {DOUBLE_MATH}.dabs (a_direction_with_respect_to_robot - transforms [i].get_heading) <= {TRIGONOMETRY_MATH}.pi_8)
				i := i + 1
			end
		end
	...
	is_enough_space_for_changing_direction: BOOLEAN
			-- Whether the robot has enough space for changing the direction.
		do
			Result := True
			across
				sensors as sensor
	...
	follow_wall_orientation (a_desired_distance_from_wall: REAL_64): REAL_64
		...
		-- TODO - TEMP - Those are our features

	is_wall_only_at_left: BOOLEAN
			-- Check if there is a wall only at left.
		do
			Result := sensors [1].is_valid_range and (not sensors [3].is_valid_range) and (not sensors [4].is_valid_range) and (not sensors [5].is_valid_range)
		end

	is_wall_only_at_right: BOOLEAN
			-- Check if there is a wall only at right.
		do
			Result := sensors [5].is_valid_range and (not sensors [3].is_valid_range) and (not sensors [2].is_valid_range) and (not sensors [1].is_valid_range)
		end
